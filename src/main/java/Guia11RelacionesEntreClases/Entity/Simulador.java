...
• La clase Simulador debe tener un método que genere un listado de alumnos manera aleatoria y lo retorne. Las 
combinaciones de nombre y apellido deben ser generadas de manera aleatoria. Nota: usar listas de tipo String para generar los nombres y los apellidos.  
...
y le asigne los nombres y los dnis de las dos listas a cada objeto Alumno. No puede haber dos alumnos con el mismo dni, pero si con el mismo nombre. 
...
En este método debemos guardar a el alumno que vota, a los alumnos a los que votó y sumarle uno a la cantidad de votos a cada alumno que reciba 
un voto, que es un atributo de la clase Alumno. Tener en cuenta que un alumno no puede votarse a sí mismo o votar más de una vez al mismo alumno.
...
• Se deben crear 5 facilitadores con los 5 primeros alumnos votados y se deben crear 5 facilitadores suplentes con los 5 segundos 
alumnos más votados. A continuación, mostrar los 5 facilitadores y los 5 facilitadores suplentes. 
 */
...
public class Simulador {
   ...
   private List<String> nombres;
    ...
    public Simulador() {
        // Inicializar las listas de nombres y apellidos
        nombres = new ArrayList<>();
        apellidos = new ArrayList<>();
        // Agregar nombres a la lista de nombres
        nombres.add("Juan");
        nombres.add("María");
        nombres.add("Luis");
        nombres.add("Ana");
        // Agregar apellidos a la lista de apellidos
        apellidos.add("Gómez");
        apellidos.add("Pérez");
    ...
    public List<String> generarListaAlumnos(int cantidad) {
    ...
    public List<String> generarListadoAlumnosConDNI(int cantidad, int rangoMin, int rangoMax) {
        

        for (int i = 0; i < cantidad; i++) {
            String dni = generarDNI(rangoMin, rangoMax);
            listadoDNI.add(dni);
        }
    ...
    private String generarDNI(int rangoMin, int rangoMax) {
        int dni = random.nextInt(rangoMax - rangoMin + 1) + rangoMin;
        return String.valueOf(dni);
    }
    
    public void generarObjetosAlumno(){
    
      for (int i = 0; i < listadoAlumnos.size(); i++) {
        String nombre = listadoAlumnos.get(i);
        String dni = listadoDNI.get(i);
